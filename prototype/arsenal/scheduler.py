...
__all__ = [
    "JointState", "run", "invalidate", "invalidate_on_push", "workflow_key",
    "missing_deps", "detect_cyclic_deps", "GhClient", "FakeGhClient",
    "RealGhClient", "CheckWriteError", "serialize_state", "deserialize_state",
]
...
class JointState:
    ...
    def from_issue_body(body: str) -> Optional["JointState"]:
        ...
        data.setdefault("joint_run_key", data.get("joint_key"))
...
def missing_deps(reports: List[Dict[str, Any]], gh: GhClient) -> List[Dict[str, str]]:
    ...
    for report in reports:
        ...
        mapping = {"driver_branch": "driver", "sim_branch": "sim", "synapse_branch": "synapse"}
        for key, repo in mapping.items():
            branch = deps.get(key)
            if not branch or report.get("repo") == repo:
                continue
...
def detect_cyclic_deps(reports: List[Dict[str, Any]]) -> Optional[List[str]]:
    ...
    for report in reports:
        ...
        src = report["repo"]
        deps = report.get("deps") or {}
        graph[src] = [repo for key, repo in mapping.items() if deps.get(key) and repo in repos_present and repo != src]
...
def find_ambiguous_deps(reports: List[Dict[str, Any]], gh: GhClient) -> Optional[Dict[str, Any]]:
    for report in reports:
        ...
        for key, repo in mapping.items():
            ...
            report_hits = [r for r in reports if r.get("repo") == repo and r.get("branch") == branch]
...
def _public_details_url(state: JointState) -> str:
    ...
    if state.workflow_runs:
        return _url(next(iter(state.workflow_runs.values())))
    return ""
...
def _safe_write_check(gh: GhClient, state: JointState, repo: str, sha: str,
                      conclusion: str, details_url: str = "", output_summary: str = "",
                      name: str = "joint-ci") -> None:
    try:
        gh.write_check(repo, sha, name, conclusion, details_url=details_url, output_summary=output_summary)
    except Exception as exc:
        _ensure_check_errors(state).append(f"{repo}@{sha}: {type(exc).__name__}({exc})")
...
def invalidate(state: JointState, gh: GhClient, reason: str = "push") -> JointState:
    ...
    for r in state.participants:
        _safe_write_check(gh, state, r["repo"], r["head_sha"], "pending", output_summary=f"invalidated:{reason}")
    return gh.upsert_issue(state)


def invalidate_on_push(reports: List[Dict[str, Any]], prior_state: Optional[JointState],
                       gh: Optional[GhClient] = None) -> Optional[JointState]:
...
def aggregate_and_write_checks(reports: List[Dict[str, Any]], merged: List[Dict[str, Any]],
                               results: Dict[str, str], state: JointState, gh: GhClient) -> str:
    """Write `joint-ci` to every participant from public results only."""
    ...
    for test in merged:
        if test.get("exclusive_to"):
            continue
        conc = results.get(workflow_key(test))
        any_pending = any_pending or conc == "pending"
        public_failed = public_failed or conc == "failure"
    if any_pending:
        for report in reports:
            _safe_write_check(gh, state, report["repo"], report["head_sha"], "pending",
                              details_url=details, output_summary="running", name="joint-ci")
        return "running"
    ...
    for report in reports:
        _safe_write_check(gh, state, report["repo"], report["head_sha"], conclusion,
                          details_url=details,
...
def _find_reusable(joint_key: str, versions: Dict[str, str], prior_state: Optional[JointState],
                   gh: GhClient) -> Optional[JointState]:
    candidates: List[JointState] = []
    if prior_state is not None:
        candidates.append(prior_state)
    issues = getattr(gh, "issues", None)
    if isinstance(issues, dict):
        candidates.extend(st for st in issues.values() if st is not prior_state)
    for st in candidates:
        if (getattr(st, "joint_key", None) == joint_key
...
def _rewrite_checks_from_reuse(reports: List[Dict[str, Any]], merged: List[Dict[str, Any]],
                               state: JointState, gh: GhClient) -> str:
    results: Dict[str, str] = {}
    for test in merged:
        key = workflow_key(test)
        run_id = state.workflow_runs.get(key)
        results[key] = gh.get_run_conclusion(run_id) if run_id else "success"
    return aggregate_and_write_checks(reports, merged, results, state, gh)


def run(reports: List[Dict[str, Any]], joint_id: str = "", gh: Optional[GhClient] = None,
        prior_state: Optional[JointState] = None) -> JointState:
    gh = gh or default_client()
    if not reports:
    ...
    if prior_state is None and hasattr(gh, "get_issue"):
        prior_state = gh.get_issue(joint_id)
    with _lock_for(joint_id):
...
def _abort_invalidated(state: JointState, joint_reports: List[Dict[str, Any]],
                       gh: GhClient, joint_id: str) -> JointState:
    reason = _gh_invalidation_reason(gh, joint_id) or state.failure_reason or "invalidated"
    state.status = "invalidated"
    state.failure_reason = reason
    _cancel_runs(gh, state)
...
def _run_locked(joint_reports: List[Dict[str, Any]], joint_id: str, gh: GhClient,
                prior_state: Optional[JointState]) -> JointState:
    ...
    if cycle:
        ...
        for report in joint_reports:
            _safe_write_check(gh, state, report["repo"], report["head_sha"], "failure",
    ...
    state.joint_run_key = make_joint_run_key(
        joint_id, versions, image=os.environ.get("JOINT_IMAGE", "default"),
        test_config_hash=cfg_hash, base_sha=os.environ.get("JOINT_BASE_SHA", ""),
    ...
    if reusable is not None:
        ...
        if reusable.status == "running" and incomplete:
            ...
            pending_left = any(gh.get_run_conclusion(rid) == "pending"
                               for rid in state.workflow_runs.values())
    ...
    for test in merged:
        ...
        if key in state.workflow_runs:
            ...
            continue
        if (not skip_reuse and prior_state is not None and prior_state.status != "invalidated"
                and prior_state.workflow_runs and key in prior_state.workflow_runs):
...
def main() -> None:
    ...
    if isinstance(data, dict) and "reports" in data:
        ...
        joint_id = os.environ.get("JOINT_ID") or ""
    gh = default_client()
    prior = gh.get_issue(joint_id) if joint_id and hasattr(gh, "get_issue") else None
